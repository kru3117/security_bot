...
async fn log_violation(state: &BotState, http: &Http, cache: &Cache, gid: GuildId, user: &User, violation: &str, reason: &str, chid: ChannelId) {
    ...
    *count += 1; let total = *count;
    let embed = CreateEmbed::default()
        .title("SECURITY VIOLATION DETECTED").color(EMBED_COLOR).timestamp(now_pht())
...
async fn instant_ban_and_rollback(state: &BotState, db: &Database, http: &Http, cache: &Cache, gid: GuildId, actor: UserId, action: &str, rollback: impl std::future::Future<Output = ()> + Send + 'static, log_extra: String) {
    ...
    tokio::spawn(async move {
        ...
        if let Some(log_id) = gid_clone.channels(&http_clone).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
            let embed = CreateEmbed::default().title("🚨 ANTI-NUKE — INSTANT BAN + ROLLBACK").color(0xFF0000).timestamp(now_pht())
...
async fn check_mass_action(state: &BotState, http: &Http, cache: &Cache, db: &Database, gid: GuildId, actor: UserId, action_type: &str) {
    ...
    if count >= MASS_ACTION_THRESHOLD {
        ...
        if let Ok(member) = gid.member(http, actor).await {
            ...
            if let Some(log_id) = gid.channels(http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                let embed = CreateEmbed::default().title(format!("🚨 ANTI MASS {}", action_type.to_uppercase())).color(0xFF0000).timestamp(now_pht())
...
async fn auto_kick_security_threat(state: &BotState, http: &Http, cache: &Cache, db: &Database, gid: GuildId, member: &Member, reason: &str) {
    ...
    let embed = CreateEmbed::default().title("AUTO-KICK").description(format!("Member {} has been automatically kicked for a security threat.", member.user.mention())).color(0xFF4500).timestamp(now_pht())
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "channel_delete").await {
            ...
            if !drain_set.contains(&actor) {
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(log_id) = gid.channels(&http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                        let embed = CreateEmbed::default().title("🚨 ANTI-NUKE — FULL NUKE ROLLBACK").color(0xFF0000).timestamp(now_pht())
                ...
                if let Some(s) = snap {
                    ...
                    let _ = gid.edit(&http, |e| {
                        e.name(&sname)
                         ...
                         .default_message_notifications(Some(serenity::model::guild::DefaultMessageNotificationLevel::from(snotif as u8)))
                         .explicit_content_filter(Some(serenity::model::guild::ExplicitContentFilter::from(secf as u8)));
                            ...
                            if let Some(ex) = existing {
                                if ex.invite_code == code && ex.channel_id == msg.channel_id {
                                    ...
                                    if let Ok(member) = gid.member(&self.http, msg.author.id).await {
                                        let _ = member.edit(&self.http, |e| e.disable_communication_until(&timeout_str)).await;
                                    }
                                    ...
                                    if let Some(log_id) = gid.channels(&self.http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                                        let embed = CreateEmbed::default().title("📢 AD SPAM DETECTED — TIMEOUT ISSUED").color(0xFF4500).timestamp(now)
...
impl Handler {
    async fn process_commands(&self, ctx: Context, msg: &Message) {
        ...
        async fn send_embed(http: &Http, ch: ChannelId, title: &str, desc: &str, color: u32) {
            let embed = CreateEmbed::default().title(title).description(desc).color(color).timestamp(now_pht());
        ...
        match cmd.as_str() {
            "antinuke" => {
                ...
                if rest.is_empty() {
                    ...
                    let mut embed = CreateEmbed::default().title("Protection Status").description(format!("Anti-Nuke + Security Protection: **{}**", status)).color(color).timestamp(now_pht());
                    if enabled {
                ...
                if setting == "on" || setting == "enable" || setting == "true" || setting == "1" {
                    ...
                    let embed = CreateEmbed::default().title("Protection Enabled").description("Anti-Nuke + Security protection is now **ACTIVE**").color(0x00FF00).timestamp(now_pht())
                    ...
                    let embed = CreateEmbed::default().title("Protection Disabled").description("Anti-Nuke + Security protection is now **INACTIVE**").color(0xFF0000).timestamp(now_pht())
            ...
            "security" => {
                ...
                if rest.is_empty() {
                    ...
                    let status = if enabled { "ENABLED" } else { "DISABLED" };
                    let color = if enabled { 0x00FF00 } else { 0xFF0000 };
                    let mut embed = CreateEmbed::default().title("Protection Status").description(format!("Anti-Nuke + Security: **{}**", status)).color(color).timestamp(now_pht());
                    if enabled {
                ...
                if setting == "on" || setting == "enable" || setting == "true" || setting == "1" {
                    self.state.protection_enabled.insert(gid, true);
                    self.db.set_protection(gid, true).await;
                    let embed = CreateEmbed::default().title("Protection Enabled").description("Anti-Nuke + Security features are now **ACTIVE**").color(0x00FF00).timestamp(now_pht())
                    ...
                    self.state.protection_enabled.insert(gid, false);
                    self.db.set_protection(gid, false).await;
                    let embed = CreateEmbed::default().title("Protection Disabled").description("Anti-Nuke + Security features are now **INACTIVE**").color(0xFF0000).timestamp(now_pht())
            ...
            "whitelistrole" => {
                ...
                if let Some(role_id) = msg.mention_roles.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Role Whitelisted").description(format!("{} has been added to the anti-nuke whitelist.", role.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "unwhitelistrole" => {
                ...
                if let Some(role_id) = msg.mention_roles.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Role Removed from Whitelist").description(format!("{} has been removed from the whitelist.", role.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "whitelistuser" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ User Whitelisted").description(format!("{} has been added to the anti-nuke whitelist.", user_mention.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "unwhitelistuser" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ User Removed from Whitelist").description(format!("{} has been removed from the whitelist.", user_mention.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "whitelistlist" => {
                ...
                let embed = CreateEmbed::default().title("🛡️ Whitelist & Bypass List").description(format!("Configuration for **{}**", guild.name)).color(EMBED_COLOR).timestamp(now_pht())
            ...
            "bypasslink" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Link Bypass Granted").description(format!("{} can now post **any link** without being filtered.", user_mention.mention())).color(0x00FF00).timestamp(now_pht())
                    ...
                    let embed = CreateEmbed::default().title("✅ Link Bypass Granted (Role)").description(format!("All members with <@&{}> can now post **any link** without being filtered.", rid.0)).color(0x00FF00).timestamp(now_pht())
            ...
            "removebypasslink" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Link Bypass Revoked").description(format!("{}'s link bypass has been removed.", user_mention.mention())).color(0xFF4500).timestamp(now_pht())
                        .field("User", user_mention.mention(), true).field("Removed by", author.mention(), true)
                        .thumbnail(user_mention.avatar_url().unwrap_or_else(|| user_mention.default_avatar_url()))
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Link Bypass"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                    ...
                    let embed = CreateEmbed::default().title("✅ Role Link Bypass Revoked").description(format!("Link bypass for <@&{}> has been removed.", rid.0)).color(0xFF4500).timestamp(now_pht())
            ...
            "setup" => {
                ...
                if let Ok(new_channel) = gid.create_channel(&self.http, |c| {
                    ...
                    let welcome = CreateEmbed::default().title("Coded by ransxmware.xyz — Logs Channel").description("This channel will receive all security notifications, anti-nuke alerts, and violation reports.").color(EMBED_COLOR).timestamp(now_pht())
            ...
            "config" => {
                ...
                let embed = CreateEmbed::default().title("Security Configuration").description("Current security settings and limits").color(EMBED_COLOR).timestamp(now_pht())
            ...
            "stats" => {
                ...
                let mut embed = CreateEmbed::default().title("Security Statistics").description(format!("Security data for **{}**", ctx.cache.guild(gid).map(|g| g.name.clone()).unwrap_or_default())).color(EMBED_COLOR).timestamp(now_pht())
            ...
            "purge" => {
                ...
                let embed = CreateEmbed::default().title("Messages Purged").description(format!("Successfully deleted **{}** messages from {}", deleted.len(), channel.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "warn" => {
                ...
                let embed = CreateEmbed::default().title("⚠️ USER WARNING").color(0xFFA500).timestamp(now_pht())
                ...
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                let dm = CreateEmbed::default().title("⚠️ You have been warned").description(format!("You have been warned in **{}**", ctx.cache.guild(gid).map(|g| g.name.clone()).unwrap_or_default())).color(0xFFA500).timestamp(now_pht())
            ...
            "mute" => {
                ...
                let embed = CreateEmbed::default().title("USER MUTED").color(0xFF4500).timestamp(now_pht())
                    .field("User", target.mention(), true).field("Moderator", author.mention(), true)
                ...
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                let dm = CreateEmbed::default().title("You have been muted").description(format!("You have been muted in **{}**", ctx.cache.guild(gid).map(|g| g.name.clone()).unwrap_or_default())).color(0xFF4500).timestamp(now_pht())
                    .field("Duration", format!("{} minutes", duration), true).field("Reason", reason, false)
                    .field("Expires", until.to_rfc3339(), true);
                let _ = target.direct_message(&self.http, CreateMessage::default().embed(dm)).await;
            }
            "unmute" => {
                ...
                let embed = CreateEmbed::default().title("USER UNMUTED").description(format!("{} has been unmuted.", target.mention())).color(0x00FF00).timestamp(now_pht())
                    .field("User", target.mention(), true).field("Moderator", author.mention(), true)
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Mute System"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            ...
            "role" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    let embed = CreateEmbed::default().title("Role Assigned").description(format!("Successfully gave {} the role {}", target.mention(), role.mention())).color(EMBED_COLOR).timestamp(now_pht())
                ...
                match reqwest::get(&format!("http://ip-api.com/json/{}", ip)).await {
                    Ok(resp) => match resp.json::<serde_json::Value>().await {
                        Ok(data) => if data["status"].as_str() == Some("success") {
                            ...
                            let mut embed = CreateEmbed::default().title("[+] IP Address Lookup").description(format!("[+] Information for IP: `{}`", ip)).color(0x0099FF).timestamp(now_pht())
            ...
            "ipgrab" => {
                ...
                let embed = CreateEmbed::default().title("IP GRAB - @Null, X").description(format!("**@GRABBED: {}**", target.name)).color(0xFF0000).timestamp(now_pht())
                ...
                let reveal = CreateEmbed::default().title("😂 Got You!").description(format!("Relax {}, that was **100% FAKE**!\nNo actual IP was captured. Discord does not expose user IPs.", target.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "status" => {
                ...
                let embed = CreateEmbed::default().title("Status Updated").description(format!("Bot status changed to **{}**", status_str.to_uppercase())).color(color).timestamp(now_pht())
            ...
            "violations" => {
                ...
                let mut embed = CreateEmbed::default().title("User Violations Report").description(format!("Security record for {}", target.mention())).color(color).timestamp(now_pht())
            ...
            "clearviolations" => {
                ...
                let embed = CreateEmbed::default().title("Violations Cleared").description(format!("Cleared all violations for {}", target.mention())).color(0x00FF00).timestamp(now_pht())
            ...
            "ping" => {
                ...
                let embed = CreateEmbed::default().title("🏓 Pong!").color(if ws_latency > api_latency { ws_color } else { api_color }).timestamp(now_pht())
            ...
            "kick" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    let dm = CreateEmbed::default().title("You have been kicked").description(format!("You were kicked from **{}**", ctx.cache.guild(gid).unwrap().name)).color(0xFF4500).timestamp(now_pht())
                    ...
                    let embed = CreateEmbed::default().title("👢 Member Kicked").description(format!("{} has been kicked from the server.", target.mention())).color(0xFF4500).timestamp(now_pht())
            ...
            "ban" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    let dm = CreateEmbed::default().title("🔨 You have been banned").description(format!("You were banned from **{}**", ctx.cache.guild(gid).unwrap().name)).color(0xFF0000).timestamp(now_pht())
                        .field("Reason", reason, false).field("Moderator", author.tag(), true);
                    let _ = target.direct_message(&self.http, CreateMessage::default().embed(dm)).await;
                    ...
                    let embed = CreateEmbed::default().title("🔨 Member Banned").description(format!("{} has been permanently banned.", target.mention())).color(0xFF0000).timestamp(now_pht())
                        .field("User", format!("{} (`{}`)", target.tag(), target.id), true).field("Moderator", author.mention(), true).field("Reason", reason, false)
                        .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Moderation"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            ...
            "unban" => {
                ...
                if let Some(user) = target {
                    ...
                    let embed = CreateEmbed::default().title("✅ Member Unbanned").description(format!("**{}** has been unbanned and can rejoin the server.", user.tag())).color(0x00FF00).timestamp(now_pht())
            ...
            "serverinfo" => {
                ...
                let mut embed = CreateEmbed::default().title(format!("☁️  {}", guild.name)).description(guild.description.clone().unwrap_or_default()).color(EMBED_COLOR).timestamp(now_pht());
            ...
            "addlink" => {
                ...
                let embed = CreateEmbed::default().title("✅ Link Whitelisted").description(format!("`{}` has been added to the allowed links.", domain)).color(0x00FF00).timestamp(now_pht())
            ...
            "removelink" => {
                ...
                self.db.save_guild_config(gid).await;
                let embed = CreateEmbed::default().title("✅ Link Removed").description(format!("`{}` has been removed from the allowed links.", domain)).color(0x00FF00).timestamp(now_pht())
            ...
            "history" => {
                ...
                let mut embed = CreateEmbed::default().title(format!("Action History — {}", target.name)).description(format!("Last {} recorded actions for {}", rows.len(), target.mention())).color(EMBED_COLOR).timestamp(now_pht())
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()));
                if rows.is_empty() {
            ...
            "help" => {
                if rest.is_empty() {
                    let embed = CreateEmbed::default().title("🛡️ Null, X : Security Menu").description("**REVSHIT**").color(EMBED_COLOR).timestamp(now_pht())
                    ...
                    if let Some((desc, usage, examples, perms)) = help_map.get(&cmd_name) {
                        let embed = CreateEmbed::default().title(format!("Help: x{}", cmd_name)).description(*desc).color(EMBED_COLOR).timestamp(now_pht())
