...
async fn log_violation(state: &BotState, http: &Http, cache: &Cache, gid: GuildId, user: &User, violation: &str, reason: &str, chid: ChannelId) {
    ...
    *count += 1; let total = *count;
    let embed = CreateEmbed::default()
        ...
        .footer(|f| f.text("Coded by ransxmware.xyz").icon_url(cache.current_user().avatar_url().unwrap_or_default()));
                ...
                if let Some(s) = snap {
                    ...
                    let _ = gid.edit(&http, |e| {
                        e.name(&sname)
                         ...
                         .verification_level(serenity::model::guild::VerificationLevel::from(sverif as u64))
                         .default_message_notifications(Some(serenity::model::guild::DefaultMessageNotificationLevel::from(snotif as u64)))
                         .explicit_content_filter(Some(serenity::model::guild::ExplicitContentFilter::from(secf as u64)));
    ...
    async fn process_commands(&self, ctx: Context, msg: &Message) {
        ...
        match cmd.as_str() {
            ...
            "serverinfo" => {
                ...
                embed = embed.footer(|f| f.text(format!("Null, X | Today at {}", now_pht().format("%I:%M %p"))).icon_url(ctx.cache.current_user().avatar_url().unwrap_or_default()));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "addlink" => {
                if !is_admin { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Administrator permission.", 0xFF0000).await; return; }
