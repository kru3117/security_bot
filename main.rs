...
impl Database {
    ...
    async fn save_guild_config(&self, gid: GuildId) {
        let sec_guard = security_config().lock().unwrap(); let cfg = &*sec_guard;
        let anti_guard = antinuke_config().lock().unwrap(); let antinuke = &*anti_guard;
        sqlx::query("INSERT OR REPLACE INTO guild_config(guild_id, threshold_count, threshold_window, punishment, max_messages_per_minute, max_duplicate_messages, max_emojis, auto_ban_threshold, link_whitelist, banned_words) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            .bind(gid.0 as i64).bind(antinuke.threshold_count as i64).bind((antinuke.threshold_window_secs * 1000.0) as i64)
            .bind(antinuke.punishment.as_str()).bind(cfg.max_messages_per_minute as i64).bind(cfg.max_duplicate_messages as i64)
            .bind(cfg.max_emojis as i64).bind(cfg.auto_ban_threshold as i64)
            .bind(serde_json::to_string(&cfg.link_whitelist).unwrap()).bind(serde_json::to_string(&cfg.banned_words).unwrap())
...
fn snap_guild(guild: &Guild) -> GuildSnapshot {
    GuildSnapshot {
        ...
        afk_channel_id: guild.afk_channel_id, afk_timeout: guild.afk_timeout, verification_level: guild.verification_level.num(),
        default_notifications: guild.default_message_notifications.num(), explicit_content_filter: guild.explicit_content_filter.num(),
...
async fn get_actor_fast(state: &BotState, http: &Http, cache: &Cache, gid: GuildId, action: &str) -> Option<UserId> {
    ...
    if let Some((actor, fetched)) = state.audit_prefetch.get(&key) {
    ...
    if let Ok(logs) = gid.audit_logs(http, Some(action_type), None, None, Some(3)).await {
        for entry in logs.entries {
            if entry.user_id == Some(http.get_current_user().await.ok()?.id) { continue; }
            let age = (now_pht().timestamp() - entry.id.created_at().unix_timestamp()).abs() as f64;
            if age < 8.0 {
                let actor = match entry.user_id { Some(u) => u, None => continue }; {
...
async fn log_violation(state: &BotState, http: &Http, cache: &Cache, gid: GuildId, user: &User, violation: &str, reason: &str, chid: ChannelId) {
    ...
    *count += 1; let total = *count;
    let embed = CreateEmbed::default()
        .title("SECURITY VIOLATION DETECTED").color(EMBED_COLOR).timestamp(now_pht().into())
        .field("User", format!("{} ({})", user.mention(), user.id), true)
        .field("Violation", violation, true).field("Total Violations", total.to_string(), true)
        .field("Reason", reason, false).thumbnail(user.avatar_url().unwrap_or_else(|| user.default_avatar_url()))
        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz").icon_url(cache.current_user().avatar_url().unwrap_or_default()));
    if let Some(log_id) = gid.channels(http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
        let _ = log_id.send_message(http, |m| m.embed(|e| { *e = embed.clone(.clone(); e }))).await;
    }
...
async fn instant_ban_and_rollback(state: &BotState, db: &Database, http: &Http, cache: &Cache, gid: GuildId, actor: UserId, action: &str, rollback: impl std::future::Future<Output = ()> + Send + 'static, log_extra: String) {
    ...
    let mut ban_set = state.ban_in_progress.entry(gid).or_insert_with(DashSet::new);
    ...
    tokio::spawn(async move {
        let _ = http_clone.ban_user(gid_clone.0, actor_clone.0, 0, Some(&format!("[Anti-Nuke] {}", action_str))).await;
        ...
        if let Some(log_id) = gid_clone.channels(&http_clone).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
            let embed = CreateEmbed::default().title("🚨 ANTI-NUKE — INSTANT BAN + ROLLBACK").color(0xFF0000).timestamp(now_pht().into())
                .field("Actor", format!("`{}`", actor_clone.0), true).field("Action", action_str, true).field("Ban", "✅ Banned", true)
                .field("Rollback", "✅ Restored", true).field("Details", log_extra, false)
                .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Anti-Nuke Rollback"));
...
async fn check_mass_action(state: &BotState, http: &Http, cache: &Cache, db: &Database, gid: GuildId, actor: UserId, action_type: &str) {
    ...
    if is_whitelisted(state, http, cache, gid, actor).await { return; }
    let now = now_pht().timestamp_millis() as f64 / 1000.0;
    let mut mass_log = state.mass_action_log.entry(gid).or_insert_with(DashMap::new);
    let timestamps = mass_log.entry(actor).or_insert_with(Vec::new);
    timestamps.push(now);
    ...
    if count >= MASS_ACTION_THRESHOLD {
        ...
        if let Ok(member) = gid.member(http, actor).await {
            ...
            let manageable: Vec<RoleId> = member.roles.iter()
                ...
                .copied()
                .collect();
            if !manageable.is_empty() { let _ = member.remove_roles(http, &manageable).await; }
            ...
            if let Some(log_id) = gid.channels(http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                let embed = CreateEmbed::default().title(format!("🚨 ANTI MASS {}", action_type.to_uppercase())).color(0xFF0000).timestamp(now_pht().into())
                    .field("Actor", format!("{} (`{}`)", member.user.tag(), actor.0), true)
                    .field("Count", count.to_string(), true).field("Window", format!("{}s", MASS_ACTION_WINDOW_SECS), true)
...
async fn auto_kick_security_threat(state: &BotState, http: &Http, cache: &Cache, db: &Database, gid: GuildId, member: &Member, reason: &str) {
    ...
    let embed = CreateEmbed::default().title("AUTO-KICK").description(format!("Member {} has been automatically kicked for a security threat.", member.user.mention())).color(0xFF4500).timestamp(now_pht().into())
        .field("Reason", reason, false).field("User ID", member.user.id.0.to_string(), true);
    if let Some(log_id) = gid.channels(http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
        let _ = log_id.send_message(http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
    }
}
...
async fn restore_channel(http: &Http, guild: &Guild, snap: &ChannelSnapshot) -> Option<ChannelId> {
    ...
    guild.create_channel(http, |c| {
        ...
        if snap_kind == ChannelType::Text {
            ...
            c.nsfw(snap_nsfw).rate_limit_per_user(snap_slowmode as u32);
        }
        c
    }).await.ok().map(|c| c.id)
}
...
async fn snapshot_all(state: &BotState, http: &Http, guild: &Guild) {
    ...
    for (id, ch) in guild.channels.iter() { state.channel_snapshots.insert(*id, snap_channel(ch)); }
    for (id, role) in guild.roles.iter() { state.role_snapshots.insert(*id, snap_role(role)); }
}

async fn poll_audit_logs(state: &BotState, http: &Http, cache: &Cache, gid: GuildId) {
...
async fn cleanup_mutes(state: &BotState, db: &Database) {
...
impl EventHandler for Handler {
    ...
    async fn webhook_update(&self, ctx: Context, guild_id: GuildId, channel_id: ChannelId) {
        ...
        let now = now_pht().timestamp_millis() as f64 / 1000.0;
        let mut entry = self.state.handled_webhook_events.entry(gid).or_insert_with(DashMap::new);
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "webhook_create").await {
            ...
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized webhook creation", async move {
                let webhooks = http.get_guild_webhooks(gid.0).await.ok()?;
                for wh in webhooks { let _ = http.delete_webhook(wh.id.0, None).await; }
    ...
    async fn channel_create(&self, ctx: Context, channel: &GuildChannel) {
        ...
        let now = now_pht().timestamp_millis() as f64 / 1000.0;
        let mut entry = self.state.handled_channel_creates.entry(gid).or_insert_with(DashMap::new);
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "channel_create").await {
            ...
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized channel creation", async move {
    ...
    async fn channel_delete(&self, ctx: Context, channel: &GuildChannel) {
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "channel_delete").await {
            ...
            let mut queue_entry = self.state.rollback_queue.entry(gid).or_insert_with(DashMap::new);
            let actor_queue = queue_entry.entry(actor).or_insert_with(Vec::new);
            ...
            if !drain_set.contains(&actor) {
                drain_set.insert(actor);
                {
                    ...
                    if !ban_set.contains(&actor) {
                        ...
                        let http = self.http.clone(); let state = self.state.clone(); let db = self.db.clone();
                        tokio::spawn(async move {
                            let _ = http.ban_user(gid.0, actor.0, 0, Some("[Anti-Nuke] Full nuke — channel deletion")).await;
                ...
                tokio::spawn(async move {
                    ...
                    let queue = { let mut e = state.rollback_queue.entry(gid).or_insert_with(DashMap::new); e.remove(&actor).map(|(_, v)| v).unwrap_or_default() };
                    ...
                    if let Some(log_id) = gid.channels(&http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                        let embed = CreateEmbed::default().title("🚨 ANTI-NUKE — FULL NUKE ROLLBACK").color(0xFF0000).timestamp(now_pht().into())
                            .field("Actor", format!("`{}`", actor.0), true).field("Action", "Mass channel deletion", true)
                            .field("Ban", "✅ Banned (before restore)", true).field("Channels Restored", queue.len().to_string(), true)
    ...
    async fn channel_update(&self, ctx: Context, old: Option<Channel>, new: Channel) {
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "channel_update").await {
            ...
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized channel edit", async move {
            ...
            }, format!("Channel **#{}** — {} → reverted.", old.name, changed)).await;
        } else { self.state.channel_snapshots.insert(new.id, snap_channel(&new)); }
    }

    async fn guild_update(&self, ctx: Context, old: Option<Guild>, new: Guild) {
        let old = match old { Some(g) => g, None => { self.state.guild_snapshots.insert(new.id, snap_guild(&new)); return; } };
        let gid = new.id;
        if !self.state.protection_enabled.get(&gid).map(|e| *e).unwrap_or(false) { self.state.guild_snapshots.insert(gid, snap_guild(&new)); return; }
        let now = now_pht().timestamp_millis() as f64 / 1000.0;
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "guild_update").await {
            if is_whitelisted(&self.state, &self.http, &ctx.cache, gid, actor).await { self.state.guild_snapshots.insert(gid, snap_guild(&new)); return; }
            ...
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized guild settings change", async move {
                if let Some(s) = snap {
                    ...
                    let _ = gid.edit(&http, |e| {
                        e.name(&sname)
                         ...
                         .verification_level(serenity::model::guild::VerificationLevel::from(sverif as u8))
                         .default_message_notifications(serenity::model::guild::DefaultMessageNotificationLevel::from(snotif as u8))
                         .explicit_content_filter(serenity::model::guild::ExplicitContentFilter::from(secf as u8));
        ...
        } else { self.state.guild_snapshots.insert(gid, snap_guild(&new)); }
    }
    ...
    async fn guild_role_create(&self, ctx: Context, role: Role) {
        ...
        let now = now_pht().timestamp_millis() as f64 / 1000.0;
        let mut entry = self.state.handled_role_events.entry(gid).or_insert_with(DashMap::new);
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "role_create").await {
            ...
            let http = self.http.clone();
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized role creation", async move {
    ...
    async fn guild_role_update(&self, ctx: Context, old: Option<Role>, new: Role) {
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "role_update").await {
            ...
            let role_id = new.id;
            let http = self.http.clone(); let state = self.state.clone();
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized role edit", async move {
            ...
            }, format!("Role **@{}** — {} → reverted.", old.name, changes)).await;
    ...
    async fn guild_role_delete(&self, ctx: Context, gid: GuildId, role_id: RoleId, role: Option<Role>) {
        ...
        if let Some(actor) = get_actor_fast(&self.state, &self.http, &ctx.cache, gid, "role_delete").await {
            ...
            instant_ban_and_rollback(&self.state, &self.db, &self.http, &ctx.cache, gid, actor, "Unauthorized role deletion", async move {
            ...
            if let Ok(logs) = gid.audit_logs(&self.http, Some(28u8), None, None, Some(5)).await {
                for entry in logs.entries {
                    ...
                    if age < 20.0 && entry.target_id == Some(new_member.user.id.0) {
                        let adder = match entry.user_id { Some(u) => u, None => continue }; {
                    ...
                    if let Ok(logs) = gid.audit_logs(&self.http, Some(25u8), None, None, Some(15)).await {
                        for entry in logs.entries {
                            ...
                            if age < 20.0 && entry.target_id == Some(new.user.id.0) {
                                let assigner = match entry.user_id { Some(u) => u, None => continue }; {
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if is_cmd {
            ...
            if timestamps.len() > RATE_LIMIT_MAX_COMMANDS {
                ...
                let embed = CreateEmbed::default().title("⏱️ Slow Down!").description(format!("{} you're sending commands too fast.\nPlease wait **{} seconds** before using commands again.", msg.author.mention(), RATE_LIMIT_COOLDOWN_SECS)).color(0xFF4500).timestamp(now.into());
        ...
        if self.state.protection_enabled.get(&gid).map(|e| *e).unwrap_or(false) {
                        ...
                        if inv_guild.id != gid.0 {
                            ...
                            if let Some(ex) = existing {
                                if ex.invite_code == code && ex.channel_id == msg.channel_id {
                                    ...
                                    let _ = msg.delete(&self.http).await;
                                    let mut spam_map = self.state.ad_spam_channels.entry(gid).or_insert_with(DashMap::new);
                                    let channels = spam_map.entry(msg.author.id).or_insert_with(Vec::new);
                                    ...
                                    let timeout_ts = serenity::model::Timestamp::from_unix_timestamp(timeout.timestamp()).unwrap_or_else(|_| serenity::model::Timestamp::now());
                                    if let Ok(member) = gid.member(&self.http, msg.author.id).await {
                                        let _ = member.edit(&self.http, |e| e.disable_communication_until(timeout_ts)).await;
                                    }
                                    ...
                                    if let Some(log_id) = gid.channels(&self.http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                                        let embed = CreateEmbed::default().title("📢 AD SPAM DETECTED — TIMEOUT ISSUED").color(0xFF4500).timestamp(now.into())
                                            .field("User", format!("{} (`{}`)", msg.author.mention(), msg.author.id), true)
                                            .field("Invite", format!("`discord.gg/{}`", code), true)
                                            .field("Channels", format!("{} channels spammed", channels.len()), true)
            ...
            // Spam detection
            let recent_times = self.state.user_message_times.get(&msg.author.id).unwrap();
            if recent_times.len() > security_config().lock().unwrap().max_messages_per_minute {
                let _ = msg.delete(&self.http).await;
                log_violation(&self.state, &self.http, &ctx.cache, gid, &msg.author, "SPAM DETECTION", &format!("Sent {} messages in 1 minute", recent_times.len()), msg.channel_id).await;
                return;
            }

            // Duplicate messages
            let recent_msgs = self.state.user_messages.get(&msg.author.id).unwrap();
            let dup_count = recent_msgs.iter().filter(|m| *m == &msg.content.to_lowercase()).count();
            if dup_count > security_config().lock().unwrap().max_duplicate_messages {
                let _ = msg.delete(&self.http).await;
            ...
            if !link_bypassed {
                ...
                for word in &security_config().lock().unwrap().banned_words {
            ...
            if emoji_count > security_config().lock().unwrap().max_emojis {
                let _ = msg.delete(&self.http).await;
                log_violation(&self.state, &self.http, &ctx.cache, gid, &msg.author, "EMOJI SPAM", &format!("Used {} emojis (limit: {})", emoji_count, security_config().lock().unwrap().max_emojis), msg.channel_id).await;
                return;
            }
            ...
            if !link_bypassed {
                ...
                for url in url_re.find_iter(&msg.content) {
                    ...
                    let allowed = security_config().lock().unwrap().link_whitelist.iter().any(|d| url_str.contains(d.as_str()));
        ...
        if let Some(caps) = null_av_re.captures(&msg.content) {
            ...
            let target = if let Some(mention) = msg.mentions.first() {
                ...
                ctx.cache.guild(gid).and_then(|g| g.members.values().find(|m| m.display_name().to_lowercase().contains(&lower) || m.user.name.to_lowercase().contains(&lower)).map(|m| &m.user))
            } else {
                None
            };
            if let Some(user) = target {
                let embed = CreateEmbed::default().color(EMBED_COLOR).image(user.avatar_url().unwrap_or_else(|| user.default_avatar_url()));
                let _ = msg.channel_id.send_message(&self.http, |m| m.content(format!("Here is {}'s avatar.", user.mention())).embed(embed)).await;
            }
            return;
        }
...
impl Handler {
    async fn process_commands(&self, ctx: Context, msg: &Message) {
        ...
        async fn send_embed(http: &Http, ch: ChannelId, title: &str, desc: &str, color: u32) {
            let embed = CreateEmbed::default().title(title).description(desc).color(color).timestamp(now_pht().into());
        ...
        match cmd.as_str() {
            "antinuke" => {
                ...
                if rest.is_empty() {
                    ...
                    let mut embed = CreateEmbed::default().title("Protection Status").description(format!("Anti-Nuke + Security Protection: **{}**", status)).color(color).timestamp(now_pht().into());
                    if enabled {
                        embed = embed.field("Active Protections", format!("• Anti Webhook Create\n• Anti Channel Create / Delete / Update\n• Anti Guild Update\n• Anti Role Create / Update / Delete\n• Message moderation (spam, caps, links, etc.)\nThreshold: **{} actions** in **{}s** → **{}**", antinuke_config().lock().unwrap().threshold_count, antinuke_config().lock().unwrap().threshold_window_secs, antinuke_config().lock().unwrap().punishment.as_str().to_uppercase()), false);
                    } else {
                        embed = embed.field("Note", "Use `xantinuke on` or `xsecurity on` to enable protection.", false);
                    }
                ...
                if setting == "on" || setting == "enable" || setting == "true" || setting == "1" {
                    ...
                    let embed = CreateEmbed::default().title("Protection Enabled").description("Anti-Nuke + Security protection is now **ACTIVE**").color(0x00FF00).timestamp(now_pht().into())
                        .field("Now Protected Against", "• Webhook creation abuse\n• Mass channel create / delete / update\n• Server (guild) settings tampering\n• Role create / update / delete spam\n• Message spam, caps, invite links, banned words\n".to_owned() + &format!("Threshold: **{} actions / {}s** → **{}**", antinuke_config().lock().unwrap().threshold_count, antinuke_config().lock().unwrap().threshold_window_secs, antinuke_config().lock().unwrap().punishment.as_str().to_uppercase()), false)
                        .field("⚠️ Important", "Whitelist trusted admins with `xwhitelistuser @user` to avoid false triggers.", false)
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Protection"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                    ...
                    let embed = CreateEmbed::default().title("Protection Disabled").description("Anti-Nuke + Security protection is now **INACTIVE**").color(0xFF0000).timestamp(now_pht().into())
            ...
            "security" => {
                ...
                if rest.is_empty() {
                    ...
                    let status = if enabled { "ENABLED" } else { "DISABLED" };
                    let color = if enabled { 0x00FF00 } else { 0xFF0000 };
                    let mut embed = CreateEmbed::default().title("Protection Status").description(format!("Anti-Nuke + Security: **{}**", status)).color(color).timestamp(now_pht().into());
                    if enabled {
                        embed = embed.field("Active Protections", "• Auto-kick unauthorized bots\n• Discord invite blocking\n• Webhook monitoring\n• Channel deletion monitoring\n• Dangerous permission monitoring\n• Message spam / caps / link filtering", false);
                    } else {
                        embed = embed.field("Available Protections", "Use `xsecurity on` to enable all protection features.", false);
                    }
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                    return;
                }
                ...
                if setting == "on" || setting == "enable" || setting == "true" || setting == "1" {
                    self.state.protection_enabled.insert(gid, true);
                    self.db.set_protection(gid, true).await;
                    let embed = CreateEmbed::default().title("Protection Enabled").description("Anti-Nuke + Security features are now **ACTIVE**").color(0x00FF00).timestamp(now_pht().into())
                        ...
                        .field("⚠️ Important", "Whitelist trusted admins with `xwhitelistuser @user` to avoid false triggers.", false)
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Security System"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                } else if setting == "off" || setting == "disable" || setting == "false" || setting == "0" {
                    self.state.protection_enabled.insert(gid, false);
                    self.db.set_protection(gid, false).await;
                    let embed = CreateEmbed::default().title("Protection Disabled").description("Anti-Nuke + Security features are now **INACTIVE**").color(0xFF0000).timestamp(now_pht().into())
            ...
            "whitelistrole" => {
                ...
                if let Some(role_id) = msg.mention_roles.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Role Whitelisted").description(format!("{} has been added to the anti-nuke whitelist.", role.mention())).color(0x00FF00).timestamp(now_pht().into())
                        .field("Role", role.mention(), true).field("Added by", author.mention(), true).field("Benefit", "Members with this role bypass anti-nuke & security checks.", false)
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            ...
            "unwhitelistrole" => {
                ...
                if let Some(role_id) = msg.mention_roles.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Role Removed from Whitelist").description(format!("{} has been removed from the whitelist.", role.mention())).color(0x00FF00).timestamp(now_pht().into())
            ...
            "whitelistuser" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ User Whitelisted").description(format!("{} has been added to the anti-nuke whitelist.", user_mention.mention())).color(0x00FF00).timestamp(now_pht().into())
            ...
            "unwhitelistuser" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ User Removed from Whitelist").description(format!("{} has been removed from the whitelist.", user_mention.mention())).color(0x00FF00).timestamp(now_pht().into())
            ...
            "whitelistlist" => {
                ...
                let wl_roles = self.state.whitelist_roles.get(&gid).map(|s| s.iter().map(|rid| guild.roles.get(rid).map(|r| r.mention()).unwrap_or_else(|| format!("<deleted role {}>", rid.0))).collect::<Vec<_>>().join("\n")).unwrap_or_else(|| "None".to_string());
                let wl_users = self.state.whitelist_users.get(&gid).map(|s| s.iter().map(|uid| guild.members.get(uid).map(|m| m.mention()).unwrap_or_else(|| format!("<user {}>", uid.0))).collect::<Vec<_>>().join("\n")).unwrap_or_else(|| "None".to_string());
                let bypass_users = self.state.link_bypass_users.get(&gid).map(|s| s.iter().map(|uid| guild.members.get(uid).map(|m| m.mention()).unwrap_or_else(|| format!("<user {}>", uid.0))).collect::<Vec<_>>().join("\n")).unwrap_or_else(|| "None".to_string());
                let bypass_roles = self.state.link_bypass_roles.get(&gid).map(|s| s.iter().map(|rid| guild.roles.get(rid).map(|r| r.mention()).unwrap_or_else(|| format!("<deleted role {}>", rid.0))).collect::<Vec<_>>().join("\n")).unwrap_or_else(|| "None".to_string());
                let embed = CreateEmbed::default().title("🛡️ Whitelist & Bypass List").description(format!("Configuration for **{}**", guild.name)).color(EMBED_COLOR).timestamp(now_pht().into())
                    ...
                    .field("🔗 Link Bypass Roles", bypass_roles, false)
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "bypasslink" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Link Bypass Granted").description(format!("{} can now post **any link** without being filtered.", user_mention.mention())).color(0x00FF00).timestamp(now_pht().into())
                        ...
                        .thumbnail(user_mention.avatar_url().unwrap_or_else(|| user_mention.default_avatar_url()))
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Link Bypass"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                    ...
                    let embed = CreateEmbed::default().title("✅ Link Bypass Granted (Role)").description(format!("All members with <@&{}> can now post **any link** without being filtered.", rid.0)).color(0x00FF00).timestamp(now_pht().into())
            ...
            "removebypasslink" => {
                ...
                if let Some(user_mention) = msg.mentions.iter().next() {
                    ...
                    let embed = CreateEmbed::default().title("✅ Link Bypass Revoked").description(format!("{}'s link bypass has been removed.", user_mention.mention())).color(0xFF4500).timestamp(now_pht().into())
                        .field("User", user_mention.mention(), true).field("Removed by", author.mention(), true)
                        .thumbnail(user_mention.avatar_url().unwrap_or_else(|| user_mention.default_avatar_url()))
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Link Bypass"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                    ...
                    let embed = CreateEmbed::default().title("✅ Role Link Bypass Revoked").description(format!("Link bypass for <@&{}> has been removed.", rid.0)).color(0xFF4500).timestamp(now_pht().into())
            ...
            "setup" => {
                ...
                if let Ok(new_channel) = gid.create_channel(&self.http, |c| {
                    ...
                    let welcome = CreateEmbed::default().title("Coded by ransxmware.xyz — Logs Channel").description("This channel will receive all security notifications, anti-nuke alerts, and violation reports.").color(EMBED_COLOR).timestamp(now_pht().into())
                        ...
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz").icon_url(ctx.cache.current_user().avatar_url().unwrap_or_default()));
            ...
            "config" => {
                if !is_admin { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Administrator permission.", 0xFF0000).await; return; }
                let enabled = self.state.protection_enabled.get(&gid).map(|e| *e).unwrap_or(false);
                let log_channel = gid.channels(&self.http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(_, c)| c.mention())).unwrap_or_else(|| "❌ Not Set".to_string());
                let embed = CreateEmbed::default().title("Security Configuration").description("Current security settings and limits").color(EMBED_COLOR).timestamp(now_pht().into())
                    .field("Protection (Anti-Nuke + Security)", if enabled { "✅ ENABLED" } else { "🔴 DISABLED" }, false)
                    .field("Logs Channel", log_channel, true)
                    .field("Anti-Nuke Settings", format!("**Threshold:**  {} actions\n**Window:**     {}s\n**Punishment:** {}", antinuke_config().lock().unwrap().threshold_count, antinuke_config().lock().unwrap().threshold_window_secs, antinuke_config().lock().unwrap().punishment.as_str().to_uppercase()), false)
                    .field("Security Limits", format!("**Messages/Minute:**    {}\n**Duplicate Messages:** {}\n**Max Emojis:**         {}\n**Auto-ban Threshold:** {} violations", security_config().lock().unwrap().max_messages_per_minute, security_config().lock().unwrap().max_duplicate_messages, security_config().lock().unwrap().max_emojis, security_config().lock().unwrap().auto_ban_threshold), false)
            ...
            "stats" => {
                ...
                let top5 = top.iter().take(5).map(|e| format!("{}: {}", ctx.cache.guild(gid).unwrap().members.get(e.key()).map(|m| m.mention()).unwrap_or_else(|| format!("<@{}>", e.key().0)), e.value())).collect::<Vec<_>>().join("\n");
                let mut embed = CreateEmbed::default().title("Security Statistics").description(format!("Security data for **{}**", ctx.cache.guild(gid).map(|g| g.name.clone()).unwrap_or_default())).color(EMBED_COLOR).timestamp(now_pht().into())
                ...
                if !top5.is_empty() { embed = embed.field("🚨 Top Violators", top5, false); }
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "purge" => {
                if !manage_msgs { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Manage Messages permission.", 0xFF0000).await; return; }
                ...
                let _ = msg.delete(&self.http).await;
                let deleted = channel.messages(&self.http, |m| m.limit(amount as u8)).await.unwrap_or_default();
                if !deleted.is_empty() { let _ = channel.delete_messages(&self.http, deleted.iter().collect::<Vec<_>>()).await; }
                let embed = CreateEmbed::default().title("Messages Purged").description(format!("Successfully deleted **{}** messages from {}", deleted.len(), channel.mention())).color(0x00FF00).timestamp(now_pht().into())
            ...
            "warn" => {
                ...
                let warning = WarningData { reason, moderator: author.id, timestamp: now_pht(), guild_id: gid };
                ...
                let embed = CreateEmbed::default().title("⚠️ USER WARNING").color(0xFFA500).timestamp(now_pht().into())
                    .field("User", target.mention(), true).field("Moderator", author.mention(), true)
                    .field("Warning Count", count.to_string(), true).field("Reason", reason, false)
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Warning System"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                let dm = CreateEmbed::default().title("⚠️ You have been warned").description(format!("You have been warned in **{}**", ctx.cache.guild(gid).map(|g| g.name.clone()).unwrap_or_default())).color(0xFFA500).timestamp(now_pht().into())
                    .field("Reason", reason, false).field("Moderator", author.tag(), true).field("Total Warnings", count.to_string(), true);
                let _ = target.direct_message(&self.http, CreateMessage::default().embed(dm)).await;
            }
            "mute" => {
                if !manage_msgs { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Manage Messages permission.", 0xFF0000).await; return; }
                ...
                let embed = CreateEmbed::default().title("USER MUTED").color(0xFF4500).timestamp(now_pht().into())
                    .field("User", target.mention(), true).field("Moderator", author.mention(), true)
                    .field("Duration", format!("{} minutes", duration), true).field("Reason", reason, false)
                    .field("Expires", until.to_rfc3339(), true)
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Mute System"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                let dm = CreateEmbed::default().title("You have been muted").description(format!("You have been muted in **{}**", ctx.cache.guild(gid).map(|g| g.name.clone()).unwrap_or_default())).color(0xFF4500).timestamp(now_pht().into())
                    .field("Duration", format!("{} minutes", duration), true).field("Reason", reason, false)
                    .field("Expires", until.to_rfc3339(), true);
                let _ = target.direct_message(&self.http, CreateMessage::default().embed(dm)).await;
            }
            "unmute" => {
                ...
                let embed = CreateEmbed::default().title("USER UNMUTED").description(format!("{} has been unmuted.", target.mention())).color(0x00FF00).timestamp(now_pht().into())
                    .field("User", target.mention(), true).field("Moderator", author.mention(), true)
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Mute System"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            ...
            "role" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    if let Some(guild) = gid.to_guild_cached(&ctx.cache) {
                        ...
                        let bot_highest = guild.members.get(&bot_id).and_then(|m| guild.member_highest_role(m)).map(|r| r.position).unwrap_or(0);
                        let author_member = guild.members.get(&author.id).cloned();
                        let author_highest = author_member.as_ref().and_then(|m| guild.member_highest_role(m)).map(|r| r.position).unwrap_or(0);
                    ...
                    let embed = CreateEmbed::default().title("Role Assigned").description(format!("Successfully gave {} the role {}", target.mention(), role.mention())).color(EMBED_COLOR).timestamp(now_pht().into())
                ...
                match reqwest::get(&format!("http://ip-api.com/json/{}", ip)).await {
                    Ok(resp) => match resp.json::<serde_json::Value>().await {
                        Ok(data) => if data["status"].as_str() == Some("success") {
                            ...
                            let mut embed = CreateEmbed::default().title("[+] IP Address Lookup").description(format!("[+] Information for IP: `{}`", ip)).color(0x0099FF).timestamp(now_pht().into())
                            ...
                            if lat != 0.0 && lon != 0.0 { embed = embed.field("[+] Google Maps", format!("[View on Maps](https://www.google.com/maps/search/?api=1&query={},{})", lat, lon), false); }
                            embed = embed.field("[+] Note", "IP geolocation is approximate and may not reflect the exact physical location.", false)
                                .footer(CreateEmbedFooter::new("IP Lookup Service").icon_url(ctx.cache.current_user().avatar_url().unwrap_or_default()));
                            let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            ...
            "ipgrab" => {
                ...
                let embed = CreateEmbed::default().title("IP GRAB - @Null, X").description(format!("**@GRABBED: {}**", target.name)).color(0xFF0000).timestamp(now_pht().into())
                ...
                let reveal = CreateEmbed::default().title("😂 Got You!").description(format!("Relax {}, that was **100% FAKE**!\nNo actual IP was captured. Discord does not expose user IPs.", target.mention())).color(0x00FF00).timestamp(now_pht().into())
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Stay safe online 💚"));
            ...
            "status" => {
                ...
                let embed = CreateEmbed::default().title("Status Updated").description(format!("Bot status changed to **{}**", status_str.to_uppercase())).color(color).timestamp(now_pht().into())
                    .field("Activity", format!("over {} servers!", server_count), false)
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Status System").icon_url(ctx.cache.current_user().avatar_url().unwrap_or_default()));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "violations" => {
                if !manage_msgs { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Manage Messages permission.", 0xFF0000).await; return; }
                ...
                let mut embed = CreateEmbed::default().title("User Violations Report").description(format!("Security record for {}", target.mention())).color(color).timestamp(now_pht().into())
                    ...
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — User Report"));
                if is_muted { if let Some(until) = self.state.muted_users.get(&target.id) { embed = embed.field("Mute Expires", until.to_rfc3339(), true); } }
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "clearviolations" => {
                if !is_admin { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Administrator permission.", 0xFF0000).await; return; }
                ...
                let embed = CreateEmbed::default().title("Violations Cleared").description(format!("Cleared all violations for {}", target.mention())).color(0x00FF00).timestamp(now_pht().into())
                    ...
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Violation Management"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "ping" => {
                let ws_latency: u128 = 0; // WebSocket latency not directly accessible from event context in 0.11.7
                let start = std::time::Instant::now();
                let sent = channel.send_message(&self.http, |m| m.content("🏓 Pinging...")).await.unwrap();
                ...
                let embed = CreateEmbed::default().title("🏓 Pong!").color(if ws_latency > api_latency { ws_color } else { api_color }).timestamp(now_pht().into())
                    .field("📡 WebSocket Latency", format!("`{}ms` — {}", ws_latency, ws_quality), true)
                    ...
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Ping Checker"));
                let _ = sent.edit(&self.http, |e| e.content("").embed(|em| { *em = embed; em })).await;
            }
            "kick" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    if let Some(guild) = gid.to_guild_cached(&ctx.cache) {
                        ...
                        let bot_highest = guild.member_highest_role(&guild.members.get(&bot_top).cloned().unwrap_or_else(|| member.clone())).map(|r| r.position).unwrap_or(0);
                        let target_highest = guild.member_highest_role(&member).map(|r| r.position).unwrap_or(0);
                        let author_highest = guild.member_highest_role(&ctx.cache.member(gid, author.id).unwrap_or_else(|| member.clone())).map(|r| r.position).unwrap_or(0);
                    ...
                    let dm = CreateEmbed::default().title("You have been kicked").description(format!("You were kicked from **{}**", ctx.cache.guild(gid).unwrap().name)).color(0xFF4500).timestamp(now_pht().into())
                        .field("Reason", reason, false).field("Moderator", author.tag(), true);
                    let _ = target.direct_message(&self.http, CreateMessage::default().embed(dm)).await;
                    ...
                    let embed = CreateEmbed::default().title("👢 Member Kicked").description(format!("{} has been kicked from the server.", target.mention())).color(0xFF4500).timestamp(now_pht().into())
                        .field("User", format!("{} (`{}`)", target.tag(), target.id), true).field("Moderator", author.mention(), true).field("Reason", reason, false)
                        .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Moderation"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                    if let Some(log_id) = gid.channels(&self.http).await.ok().and_then(|ch| ch.into_iter().find(|(_, c)| c.name == "security-logs").map(|(id, _)| id)) {
                        if log_id != channel { let _ = log_id.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(.clone(); e }))).await; }
                    }
                } else { let _ = send_embed(&self.http, channel, "❌ User Not Found", "Could not find that member.", 0xFF0000).await; }
            }
            "ban" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    let dm = CreateEmbed::default().title("🔨 You have been banned").description(format!("You were banned from **{}**", ctx.cache.guild(gid).unwrap().name)).color(0xFF0000).timestamp(now_pht().into())
                        .field("Reason", reason, false).field("Moderator", author.tag(), true);
                    let _ = target.direct_message(&self.http, CreateMessage::default().embed(dm)).await;
                    ...
                    let embed = CreateEmbed::default().title("🔨 Member Banned").description(format!("{} has been permanently banned.", target.mention())).color(0xFF0000).timestamp(now_pht().into())
                        .field("User", format!("{} (`{}`)", target.tag(), target.id), true).field("Moderator", author.mention(), true).field("Reason", reason, false)
                        .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()))
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — Moderation"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            ...
            "unban" => {
                ...
                if let Some(user) = target {
                    ...
                    let embed = CreateEmbed::default().title("✅ Member Unbanned").description(format!("**{}** has been unbanned and can rejoin the server.", user.tag())).color(0x00FF00).timestamp(now_pht().into())
            ...
            "av" => {
                let target = if let Some(u) = msg.mentions.iter().next() { Some(u) } else if !rest.is_empty() { let name = rest.join(" ").to_lowercase(); ctx.cache.guild(gid).and_then(|g| g.members.values().find(|m| m.display_name().to_lowercase().contains(&name) || m.user.name.to_lowercase().contains(&name)).map(|m| &m.user)) } else { None };
                if let Some(user) = target {
                    let embed = CreateEmbed::default().color(EMBED_COLOR).image(user.avatar_url().unwrap_or_else(|| user.default_avatar_url()));
                    let _ = channel.send_message(&self.http, |m| m.content(format!("Here is {}'s avatar.", user.mention())).embed(embed)).await;
            ...
            "serverinfo" => {
                ...
                let boost_str = format!("{} Boost{} (Level {})", boost_count, if boost_count != 1 { "s" } else { "" }, boost_level);
                let emojis: String = guild.emojis.values().map(|e| e.to_string()).collect();
                let mut embed = CreateEmbed::default().title(format!("☁️  {}", guild.name)).description(guild.description.clone().unwrap_or_default()).color(EMBED_COLOR).timestamp(now_pht().into());
                if let Some(icon) = guild.icon_url() { embed = embed.thumbnail(icon); }
                if let Some(banner) = guild.banner_url() { embed = embed.image(banner); }
                embed = embed.field("Server Owner", format!("{} ({})", owner.mention(), owner.name), false)
                ...
                if !emojis.is_empty() { embed = embed.field("Emoji List", emojis, false); }
                embed = embed.footer(CreateEmbedFooter::new(format!("Null, X | Today at {}", now_pht().format("%I:%M %p"))).icon_url(ctx.cache.current_user().avatar_url().unwrap_or_default()));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "addlink" => {
                if !is_admin { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Administrator permission.", 0xFF0000).await; return; }
                ...
                let embed = CreateEmbed::default().title("✅ Link Whitelisted").description(format!("`{}` has been added to the allowed links.", domain)).color(0x00FF00).timestamp(now_pht().into())
                    .field("Domain", format!("`{}`", domain), true).field("Added by", author.mention(), true)
                    .field("Total Allowed", security_config().lock().unwrap().link_whitelist.len().to_string(), true)
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "removelink" => {
                ...
                self.db.save_guild_config(gid).await;
                let embed = CreateEmbed::default().title("✅ Link Removed").description(format!("`{}` has been removed from the allowed links.", domain)).color(0x00FF00).timestamp(now_pht().into())
            ...
            "history" => {
                ...
                let mut embed = CreateEmbed::default().title(format!("Action History — {}", target.name)).description(format!("Last {} recorded actions for {}", rows.len(), target.mention())).color(EMBED_COLOR).timestamp(now_pht().into())
                    .thumbnail(target.avatar_url().unwrap_or_else(|| target.default_avatar_url()));
                if rows.is_empty() {
                    embed = embed.field("No History", "No recorded actions for this user in this server.", false);
                } else {
                    for row in rows {
                        let dt = DateTime::parse_from_rfc3339(row.get::<&str, _>(2)).unwrap();
                        embed = embed.field(format!("`{}` — {}", row.get::<&str, _>(0), dt.format("%Y-%m-%d %H:%M UTC")), row.get::<&str, _>(1), false);
                    }
                }
                ...
                embed = embed.field("User ID", format!("`{}`", target.id), true)
                    .field("Violations", violations.to_string(), true).field("Warnings", warnings.to_string(), true)
                    .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz — History"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
            }
            "help" => {
                if rest.is_empty() {
                    let embed = CreateEmbed::default().title("🛡️ Null, X : Security Menu").description("**REVSHIT**").color(EMBED_COLOR).timestamp(now_pht().into())
                        ...
                        .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz")).thumbnail(ctx.cache.current_user().avatar_url().unwrap_or_default());
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                } else {
                    ...
                    if let Some((desc, usage, examples, perms)) = help_map.get(&cmd_name) {
                        let embed = CreateEmbed::default().title(format!("Help: x{}", cmd_name)).description(*desc).color(EMBED_COLOR).timestamp(now_pht().into())
                            ...
                            .footer(CreateEmbedFooter::new("Coded by ransxmware.xyz Help System"));
                        let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
