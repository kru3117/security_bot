...
async fn check_mass_action(state: &BotState, http: &Http, cache: &Cache, db: &Database, gid: GuildId, actor: UserId, action_type: &str) {
    ...
    if count >= MASS_ACTION_THRESHOLD {
        timestamps.clear();
        if let Ok(mut member) = gid.member(http, actor).await {
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if is_cmd {
            ...
            if timestamps.len() > RATE_LIMIT_MAX_COMMANDS {
                ...
                if let Ok(mut sent) = msg.channel_id.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await { tokio::time::sleep(Duration::from_secs(5)).await; let _ = sent.delete(&self.http).await; }
                return;
            }
        }
                        ...
                        if inv_guild.id != gid.0 {
                            ...
                            if let Some(ex) = existing {
                                if ex.invite_code == code && ex.channel_id == msg.channel_id {
                                    ...
                                    if let Ok(mut sent) = msg.channel_id.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await { tokio::time::sleep(Duration::from_secs(8)).await; let _ = sent.delete(&self.http).await; }
                                    return;
                                } else {
                                    let _ = msg.delete(&self.http).await;
                                    ...
                                    if let Ok(mut sent) = msg.channel_id.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await { tokio::time::sleep(Duration::from_secs(15)).await; let _ = sent.delete(&self.http).await; }
                                    return;
                                }
                            } else {
    ...
    async fn process_commands(&self, ctx: Context, msg: &Message) {
        ...
        match cmd.as_str() {
            ...
            "purge" => {
                ...
                if let Some(mut s) = sent { tokio::time::sleep(Duration::from_secs(5)).await; let _ = s.delete(&self.http).await; }
            }
            "warn" => {
                if !manage_msgs { let _ = send_embed(&self.http, channel, "❌ Missing Permissions", "You need Manage Messages permission.", 0xFF0000).await; return; }
                ...
                let embed = CreateEmbed::default().title("⚠️ USER WARNING").color(0xFFA500u32).timestamp(now_pht())
                    ...
                    .thumbnail(target_avatar.clone())
                    .footer(|f| f.text("Coded by ransxmware.xyz — Warning System"));
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                let author_tag = author.tag();
                let dm = CreateEmbed::default().title("⚠️ You have been warned").description(format!("You have been warned in **{}**", guild_name)).color(0xFFA500u32).timestamp(now_pht())
            ...
            "mute" => {
                ...
                let embed = CreateEmbed::default().title("USER MUTED").color(0xFF4500u32).timestamp(now_pht())
                    .field("User", target.mention(), true).field("Moderator", author.mention(), true)
                ...
                let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
                let dm = CreateEmbed::default().title("You have been muted").description(format!("You have been muted in **{}**", guild_name)).color(0xFF4500u32).timestamp(now_pht())
            ...
            "kick" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    let target_avatar = target.avatar_url().unwrap_or_else(|| target.default_avatar_url());
                    let dm = CreateEmbed::default().title("You have been kicked").description(format!("You were kicked from **{}**", guild_name)).color(0xFF4500u32).timestamp(now_pht())
                    ...
                    let embed = CreateEmbed::default().title("👢 Member Kicked").description(format!("{} has been kicked from the server.", target.mention())).color(0xFF4500u32).timestamp(now_pht())
            ...
            "ban" => {
                ...
                if let Ok(member) = gid.member(&self.http, target.id).await {
                    ...
                    let target_tag = target.tag();
                    let target_avatar = target.avatar_url().unwrap_or_else(|| target.default_avatar_url());
                    let dm = CreateEmbed::default().title("🔨 You have been banned").description(format!("You were banned from **{}**", guild_name)).color(0xFF0000u32).timestamp(now_pht())
                        .field("Reason", reason.clone(), false).field("Moderator", author_tag.clone(), true);
                    let _ = target.direct_message(&self.http, |m| m.embed(|e| { *e = dm.clone(); e })).await;
                    ...
                    let embed = CreateEmbed::default().title("🔨 Member Banned").description(format!("{} has been permanently banned.", target.mention())).color(0xFF0000u32).timestamp(now_pht())
                        .field("User", format!("{} (`{}`)", target_tag, target.id), true).field("Moderator", author.mention(), true).field("Reason", reason, false)
                        .thumbnail(target_avatar)
                        .footer(|f| f.text("Coded by ransxmware.xyz — Moderation"));
                    let _ = channel.send_message(&self.http, |m| m.embed(|e| { *e = embed.clone(); e })).await;
